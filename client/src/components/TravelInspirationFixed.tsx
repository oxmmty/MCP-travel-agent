...
import { ExternalLink, Camera, Play, RefreshCw, Heart } from "lucide-react";
...
// Echte YouTube-Video-IDs für verschiedene Destinationen
...
function InspirationImage({ photo, className }: { photo: UnsplashPhoto; className?: string }) {
  ...
  console.log('Rendering image:', photo.url);
...
export function TravelInspiration({ destination, className }: TravelInspirationProps) {
  ...
  const { data: photosData, isLoading: photosLoading, refetch: refetchPhotos, error } = useQuery<UnsplashResponse>({
    queryKey: [`/api/unsplash/destination/${destination}`, refreshKey],
    enabled: !!destination,
  });

  // Debug logs
  console.log('TravelInspiration Debug:', {
    destination,
    isLoading: photosLoading,
    hasData: !!photosData,
    photosCount: photosData?.photos?.length,
    error: error
  });
